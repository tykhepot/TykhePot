...
async function main() {
  ...
  // Build initialize instruction manually with correct Anchor discriminator
  ...
  const referralBN = BigInt(200_000_000) * BigInt(10 ** DECIMALS); // 200M TPOT
  const paramsBuf = Buffer.alloc(16);
  paramsBuf.writeBigUInt64LE(BigInt(0), 0);       // pre_pool = 0
  paramsBuf.writeBigUInt64LE(referralBN, 8);       // referral_pool = 200M TPOT
  const instructionData = Buffer.concat([discriminator, paramsBuf]);
  ...
  const initIx = new web3.TransactionInstruction({
    ...
    keys: [
      ...
      { pubkey: adminKeypair.publicKey,        isSigner: false, isWritable: false }, // platformWallet
  ...
  const initTxObj = new web3.Transaction().add(initIx);
  const initTx = await web3.sendAndConfirmTransaction(connection, initTxObj, [adminKeypair], {
    commitment: "confirmed",
  });
